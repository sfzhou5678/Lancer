...
public class JavaLexer implements Lexer {
    ...
    public Stream<Stream<String>> lex(List<String> lines) {
        ...
        List<List<String>> lineTokens = (List<List<String>>) tokenizeLines(text).get("lineTokens");
        return lineTokens.stream().map(List::stream);
    }

    public HashMap<String, Object> tokenizeLines(String text) {
        return tokenizeLines(text, false);
    }

    public HashMap<String, Object> tokenizeLines(String text, boolean tokenizeComments) {
        IScanner scanner = ToolFactory.createScanner(tokenizeComments, false, true, "1.8");
